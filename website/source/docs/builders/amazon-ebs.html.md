...
-   `skip_register_ami` (boolean) - Set to `true` if you want to skip creation
    of an AMI. This skips stopping the instance and creation of the AMI and
    jumps directly to instance termination after provisioning. This may be
    useful if the instance provisioning is producing an artifact other than an
    AMI. The `ami_name` option can be omitted if this is set.  Default `false`.
